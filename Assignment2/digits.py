...
def get_loss(w1, w2, x, y, w_train, b_train):
    # y=trainLabels
    # x=trainData
    # np.random.seed(1)
...
def grad_descent_6b(x, y, init_w, init_b, alpha, iterations):  # ,WW
    ...
    Weights_m = list()
    Weights_m.append((w1, w2))
    print(type(Weights_m))
...
def testPart4():
    ...
    w_train, b_train = grad_descent(NLL, grad_NLL_W, grad_NLL_b, trainData, trainLabels, W, b, 0.00001, 500)
...
def Plot_Contour(Weights, Weights_m, x, y, w_train, b_train):
    gd_traj = Weights
    mo_traj = Weights_m
    ...
    for i, w1 in enumerate(w1s):
        for j, w2 in enumerate(w2s):
            C[j, i] = get_loss(w1, w2, x, y, w_train, b_train)
